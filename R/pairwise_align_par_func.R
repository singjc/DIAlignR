pairwise_align_par_func <- function( oswdata_runpair_data, XICs.ref, mzPntrs, function_param_input ){
  ...
  if( identifying ){
    ...
    if ( any(grepl(paste0(".*", filenames$runs[which(rownames(filenames) %in% ref)], ".*"), oswFile_subset$filename)) & any(grepl(paste0(".*", filenames$runs[which(rownames(filenames) %in% eXp)], ".*"), oswFile_subset$filename)) ) {
      ...
      chromIndices <- selectChromIndices(oswdata, runname = ref, analyte = analyte, product_mz_filter_list=procuct_mz_intersect, return_index=return_index, keep_all_detecting=keep_all_detecting)
      chromIndices <- selectChromIndices(oswdata, runname = ref, analyte = analyte, product_mz_filter_list=procuct_mz_intersect, return_index=return_index, keep_all_detecting=keep_all_detecting)
      tictoc::tic()
  ...
  if(!is.null(chromIndices)){
    ...
    if(!is.null(eXp_feature)){
      ...
      analyte_run_pair_results <- data.table::data.table( transition_group_id = analyte,
                                                          ...
                                                          RTo = subset( oswdata_runpair_data, subset = run_type=="eXp" & m_score==min(m_score), select = RT )[[1]] %>% as.character(),
                                                          leftWidtho = subset( oswdata_runpair_data, subset = run_type=="eXp" & m_score==min(m_score), select = leftWidth )[[1]] %>% as.character(),
                                                          rightWidtho = subset( oswdata_runpair_data, subset = run_type=="eXp" & m_score==min(m_score), select = rightWidth )[[1]] %>% as.character(),
                                                          Intensityo = subset( oswdata_runpair_data, subset = run_type=="eXp" & m_score==min(m_score), select = Intensity )[[1]] %>% as.character(),
